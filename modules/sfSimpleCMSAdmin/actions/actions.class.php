...
class sfSimpleCMSAdminActions extends autosfSimpleCMSAdminActions
{
  ...
  {
    $page = Doctrine::getTable('sfSimpleCMSPage')->find($this->getRequestParameter('id'));
  ...
  public function executeCreateRootPage()
  {
    ...
    $page->setSlug($this->getRequestParameter('slug'));
    $page->setTemplate($this->getRequestParameter('template'));
    Doctrine::getTable('sfSimpleCMSPage')->getTree()->createRoot($page);
  ...
  {
    parent::executeList();
    $this->getRequest()->setAttribute('page_names', Doctrine::getTable('sfSimpleCMSPage')->getAllPagesWithLevel());
  }
}
